...
class BazaraApi
{
    ...
    /*
    |--------------------------------------------------------------------------
    | دریافت داده‌های همگام‌سازی از API بازارا
    |--------------------------------------------------------------------------
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | رفع مغایرت داده‌های محلی با سرور
    |--------------------------------------------------------------------------
    | ردیف‌های جداول همگام‌سازی را جمع‌آوری کرده و برای هر موجودیت درخواست
    | رفع مغایرت به API ارسال می‌کند.
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | احراز هویت و مدیریت توکن API
    |--------------------------------------------------------------------------
    | در صورت معتبر بودن توکن ذخیره‌شده، همان را بازمی‌گرداند؛ در غیر این صورت
    | با اطلاعات افزونه وارد می‌شود و توکن جدید را ذخیره می‌کند.
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | همگام‌سازی دسته‌بندی‌های ووکامرس
    |--------------------------------------------------------------------------
    | دسته‌بندی‌های محصول و داده‌های تکمیلی بازارا را به taxonomy محصول تبدیل
    | می‌کند و وضعیت همگام‌سازی هر دسته را به‌روز می‌سازد.
    */
        ...
        try {
            ...
            foreach ($product_result['message']['Visitors'] as $visitor) {
                if ($visitor['IsActive'] == false)
                    continue;
                if ($selectedVisitor != $visitor['VisitorId'])
                    continue;
            ...
            if (!empty($Banks)) {
                usort($Banks, function ($item1, $item2) {
                    if ($item1['RowVersion'] == $item2['RowVersion'])
                        return 0;
    ...
    /*
    |--------------------------------------------------------------------------
    | همگام‌سازی محصولات جدید
    |--------------------------------------------------------------------------
    | محصولات انتخابی یا بازه مشخص‌شده را از داده‌های بازارا دریافت و در
    | ووکامرس ایجاد یا به‌روزرسانی می‌کند.
    */
    public function start_sync_new_product($min = 0, $max = 20, $schd = false, $selected_ids = null)
    {
        try {
            ...
            $Voptions = get_bazara_visitor_settings();
            $VoptionBarcode = $Voptions['barcode'] == 1 || $Voptions['barcode'];
            ...
            foreach ($products as $product) {
                ...
                $barcode = is_object($product)
                    ? ($product->barcode ?? null)
                    : ($product['barcode'] ?? null);

                if ($VoptionBarcode && empty($barcode)) {
                    continue;
                }
                ...
                foreach ($productDetail as $price) {
                    ...
                    if ((int) $price->Deleted === 1) {

                        $variation = get_product_variation($not, $price->ProductDetailId);
                    ...
                    bz_cleanup_duplicate_variations_from_report_v2($price->ProductDetailId, 100);


                    if (class_exists("bazara_second_count"))
                        $price->Count1 = $price->Count2;

                    if (empty($price->ProductDetailId))
                        continue;
                    if ($price->NotVariation == 1 && !empty($price->Properties))
                        continue;
                    ...
                    if (!empty($price->Properties)) {
                        ...
                        foreach ($properties as $property) {
                            ...
                            foreach ($obj as $std) {
                                if ($std['C'] != $property->PropertyDescriptionCode)
                                    continue;
                    ...
                    foreach ($properties as $property) {
                        ...
                        foreach ($product_items['Objects'] as $prop => $value) {
                            foreach ($value as $var) {
                                if ($var['C'] != $property->PropertyDescriptionCode)
                                    continue;
                                if (class_exists("bazara_default_properties")) {
                                    if (empty($var['V']))
                                        $var['V'] = "پیش فرض";
                ...
                // if ($r['success']) {
                //     bazara_sync_log(
                //         'SUCCESS | ProductId=' . $product->ProductId .
                //         ' | ProductCode=' . $product->ProductCode
                //     );
                // } else {
                //     bazara_sync_log(
                //         'FAILED | ProductId=' . $product->ProductId .
                //         ' | ProductCode=' . $product->ProductCode .
    ...
    /*
    |--------------------------------------------------------------------------
    | دریافت و ذخیره موجودیت‌های بازارا
    |--------------------------------------------------------------------------
    | اطلاعات پایه مانند محصول، قیمت، موجودی، ویژگی و داده‌های تکمیلی را به
    | صورت افزایشی دریافت کرده و در جداول محلی ذخیره می‌کند.
    */
        ...
        switch ($type) {
            ...
            case 'Products':
                ...
                if (!empty($Products)) {
                    ...
                    foreach ($Products as $product) {
                        ...
                        if ($product['RowVersion'] == $latesVersion)
                            continue;
            ...
            case 'ProductDetails':
                ...
                if (!empty($ProductDetails)) {
                    ...
                    foreach ($ProductDetails as $productdetail) {
                        $latesVersion = (empty($latest_rowVersion) ? 0 : ($latest_rowVersion));
                        if ($productdetail['RowVersion'] == $latesVersion)
                            continue;
            ...
            case 'Persons':
                ...
                if (!empty($Peoples)) {
                    ...
                    foreach ($Peoples as $People) {
                        $latesVersion = (empty($latest_rowVersion) ? 0 : ($latest_rowVersion));
                        if ($People['RowVersion'] == $latesVersion)
                            continue;
    ...
    |--------------------------------------------------------------------------
    | تصاویر حذف‌شده را پاک می‌کند و تصاویر جدید را به تصویر شاخص یا گالری
    | محصولات ووکامرس اضافه می‌کند.
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | همگام‌سازی اشخاص و گروه‌های اشخاص
    |--------------------------------------------------------------------------
    | اشخاص دریافت‌شده از بازارا را پردازش و حساب‌های کاربری و نقش‌های
    | ووکامرس مرتبط را ایجاد یا به‌روزرسانی می‌کند.
    */
    ...
    {
        if (empty($personGroupID) || empty($role_groups))
            return null;
    ...
    /*
    |--------------------------------------------------------------------------
    | ارسال سفارش‌های ووکامرس به بازارا
    |--------------------------------------------------------------------------
    | سفارش‌های جدید و سفارش‌های باقی‌مانده را پیدا کرده و پس از ارسال موفق
    | وضعیت آخرین سفارش همگام‌شده را ثبت می‌کند.
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | آماده‌سازی و ارسال یک سفارش به API
    |--------------------------------------------------------------------------
    | اطلاعات مشتری، اقلام سفارش، موجودی، پرداخت و جزئیات تحویل را به ساختار
    | قابل ارسال بازارا تبدیل می‌کند و پاسخ آن را پردازش می‌کند.
    */
    public function bazara_save_order($order_id, $token)
    {
        ...
        if (count($product_orders['orderDetails']) <> (count($order->get_items()) - $skipped_zero_quantity_items)) {
    ...
    private function get_selected_shipping_person_id($shipping_method)
    {
        ...
        // تبدیل stdClass به string
        if (is_object($shipping_method) && isset($shipping_method->shipping_method)) {
            $shipping_method = (string) $shipping_method->shipping_method;
        } else if (!is_string($shipping_method)) {
            $shipping_method = '';
        }
        ...
        if (!empty($product_result['message']['Products'])) {
            ...
            foreach ($Products as $product) {
                ...
                $product_items = [
                    ...
                    'ProductCode' => $product['ProductCode'],
                    'ProductName' => $product['Name'],
                    'Status' => $this->visitor_settings['publishStatus'] ?? 'publish',
                    'Category' => $product['ProductCategoryId'],
                    'TaxPercent' => ($product['TaxPercent'] == '-1' ? 0 : $product['TaxPercent']),
                    'ChargePercent' => ($product['ChargePercent'] == '-1' ? 0 : $product['ChargePercent']),
                    'tax' => ($taxClass ?: ''),
